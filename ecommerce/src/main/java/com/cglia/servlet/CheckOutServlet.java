        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (cart_list != null && auth != null) {
                ...
                if (auth == null) {
                    ...
                    request.getSession().setAttribute("checkoutMessage", "Please log in to check out.");
                    response.sendRedirect("login.jsp");
                } else {
                    // Redirect to cart page with message
                    request.setAttribute("message", "Your cart is empty. Please add items to proceed to checkout.");
